    ...
    while 1:
        ...
        # 60 % chance to send data
            ...
            temperature = random.randint(
                desired_temperature - 3, desired_temperature + 3)
            # Generate humidity based on current himudity
            humidity = random.randint(current_humidity - 5 if current_humidity - 5 >
                                      0 else 0, current_humidity + 5 if current_humidity + 5 < 100 else 100)
    ...
    while True:
        ...
        while not manager_queue.empty():
            ...
            if not available_technicians.empty():
                ...
                threading.Thread(target=technician_fix_freezer, args=(
                    available_technicians.get(), manager_queue.get(),), daemon=True).start()
...
if __name__ == "__main__":
    ...
    # Generate freezers
    freezers = [Freezer(freezer_thing_ids[i], freezer_numbers[i])
                for i in range(FREEZERS)]

    # Generate freezer threads
    freezer_threads = []
    for freezer in freezers:
        ft = threading.Thread(target=freezer_functioning,
                              args=(freezer,), daemon=True)
